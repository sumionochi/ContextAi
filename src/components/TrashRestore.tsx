...
const TrashRestore = () => {
  ...
  //http://localhost:3000/workspace/4c594ef4-e29a-4e71-9bd2-ac207afb00af/c205f8e8-ef41-4933-bba4-bb51a4839f81/0d4f26af-9f50-4d10-85a9-6d2b09767425
  return (
    <section>
      {!!folders.length && (
        <>
          ...
          {folders.map((folder) => (
            <Link
              ...
              href={`/workspace/${folder.workspaceId}/${folder.id}`}
            >
              <article>
      ...
      {!!files.length && (
        <>
          ...
          {files.map((file) => (
            <Link
              className=" hover:bg-muted rounded-md p-2 flex items-center justify-between"
              href={`/workspace/${file.workspaceId}/${file.folderId}/${file.id}`}
            >
              <article>
                <aside className="flex items-center gap-2">
                  <FileIcon />
